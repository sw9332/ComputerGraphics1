...
bool ModelClass::InitializeInstanceBuffer(ID3D11Device* device, int maxInstances)
{
	D3D11_BUFFER_DESC instanceBufferDesc;
	D3D11_SUBRESOURCE_DATA instanceData;
	HRESULT result;

	m_instanceCount = maxInstances;

	// 인스턴싱용 임시 데이터 배열 생성 (X, Y, Z 좌표만 담는 구조)
	struct InstanceType { XMFLOAT3 position; };
	InstanceType* instances = new InstanceType[m_instanceCount];
	for (int i = 0; i < m_instanceCount; i++) { instances[i].position = XMFLOAT3(0.0f, 0.0f, 0.0f); }

	// 버퍼 기술서 설정
	instanceBufferDesc.Usage = D3D11_USAGE_DYNAMIC;
	instanceBufferDesc.ByteWidth = sizeof(InstanceType) * m_instanceCount;
	instanceBufferDesc.BindFlags = D3D11_BIND_VERTEX_BUFFER; // ★ 인스턴스 버퍼도 결국 정점 버퍼 슬롯에 들어감
